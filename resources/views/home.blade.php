     ...
     if($conta != 0){
          ...
          //contar las tareas asociadas a cada capitulo
            ...
            foreach($tarusuario as $tusu){
              $valor = CapModel::where('idusu', $userauth_id)
                     ->where('idcap', $tusu->capitulo)->where('numtareas', $tusu->total)->where('estado', '0')
                     ->select('id')
                     ->first();
              if(!empty($valor)){
  ...
  <p style="padding-bottom:10rem;">
    ...
    <!---se completo-->
     @if(empty($diferentes))
      ...
      @if($contar2 == 0)
        <div class="text-center">
          <a href="/capitulos/{{$capconsul}}" class="btn btn-primary mx-auto">Comenzar capítulo {{$capconsul}}</a>
        </div>
      @else
      <div class="text-center">
